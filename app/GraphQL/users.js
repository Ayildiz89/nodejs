    ...
    Query: {
        ...
        current_student_count: async (obj, args, context, info) => {
            ...
            if(tk_status){
                ...
                let nextDate = _.clone(today);
                nextDate.setDate(today.getDate()+20);
                //console.log("----------->",[today, new Date(nextDate)])
                ...
                const event_users = await db.event_users.findAll({
                    where: {
                        event_id: {
                            [Op.in]:events_id
                        }
                ...
                const filtered_users_id = event_users.map(eu=>eu.user_id).filter((value, index, self) => self.indexOf(value) === index)
                const count = filtered_users_id.length;
                return count
            } else {
                throw new ApolloError("token is required",1000)
            }
